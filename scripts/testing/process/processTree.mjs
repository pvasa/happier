...
function collectTreeTargets(rootPid, additionalPids) {
  ...
  for (const currentRootPid of roots) {
    if (!Number.isInteger(currentRootPid) || currentRootPid <= 0) continue;
    const entry = byPid.get(currentRootPid);
    if (entry?.pgid && entry.pgid > 0 && entry.pgid === currentRootPid) {
