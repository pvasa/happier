---
title: Codex
description: Configure Codex in Happier, including ACP as the default backend, installables management, local control, and resume behavior.
---
...
- **Codex ACP**: the default Codex backend in Happier and the backend used for Codex resume.
- **Codex MCP**: still available for normal fresh sessions.
...
- Happier now defaults Codex to **ACP**
- old `mcp_resume` behavior is gone
- if ACP is unavailable for a **new** session, Happier may fall back to MCP with a warning
- if you explicitly try to **resume** a Codex session and ACP is unavailable, Happier errors instead of falling back

If you manually switch Codex to plain `mcp` in settings, new sessions can still run, but Codex sessions are no longer resumable from Happier.
...
- **resume reliability** — Codex resume requires ACP to be available on the machine
...
If you want predictable Codex resume on a machine, keep Codex ACP installed there and leave automatic updates enabled unless you have a specific reason not to.
...
Codex resume in Happier is **ACP-only**.

That means an inactive Codex session can be resumed only when:

- the session has valid Codex resume metadata
- Codex ACP is enabled in your Happier settings
- ACP is actually available when you resume
