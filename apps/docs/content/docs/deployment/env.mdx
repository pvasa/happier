...
- Legacy aliases prefixed with `HAPPY_` are still accepted where listed, but `HAPPIER_` is the canonical prefix.
- Provider billing/subscription secrets are intentionally omitted here; use the provider-specific docs for those keys.
