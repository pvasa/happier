...
By default, the Proxmox helper script installs Happier using the official **self-host runtime** installers (`hstack self-host` under the hood), which support stable/preview/dev channels and in-place updates.
...
- Or update from inside the container:
  - `hstack self-host update --mode system --channel stable|preview|dev`
