...
- `dev` (rolling public nightly / newest public branch builds)
...
| **Server runtime installer** (`https://happier.dev/install-server`) | `curl …install-server \| bash` (Linux) | You rerun it | No | Re-run the installer to replace the `happier-server` binary (rolling tags `server-stable` / `server-preview` / `server-dev`) |
...
Public dev builds use the same updater model, but point at the `dev` feed instead.
...
- the **server runtime binary** (from the rolling release tag for your channel: `server-stable` / `server-preview` / `server-dev`)
- the **UI web bundle** (from `ui-web-stable` / `ui-web-preview` / `ui-web-dev`), unless you disabled UI
