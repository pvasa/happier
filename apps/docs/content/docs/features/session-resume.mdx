...
- Codex now defaults to **ACP** in Happier
- legacy `mcp_resume` behavior is gone
- if you opt Codex back to plain **MCP**, Codex sessions are no longer resumable
- for new Codex sessions, Happier may fall back from ACP to MCP if ACP is unavailable
- for an explicit **resume**, Happier stays strict and errors instead of falling back

So for Codex, “can I resume this session?” depends both on the session metadata and on whether Codex ACP is enabled in your settings.
