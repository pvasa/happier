...
- Start Metro for the Expo Dev Client with native E2E selectors enabled: `EXPO_PUBLIC_HAPPIER_NATIVE_E2E_TEST_IDS=1` (so Maestro can target `testID` via accessibility labels).
