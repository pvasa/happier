...
function formatReleaseChannel(channel: string): string {
  ...
  if (normalized === 'publicdev' || normalized === 'dev') return chalk.cyan('dev');
  return channel;
}
...
function formatAutomaticStartupLines(params: Readonly<{
  ...
  if (inventory.length === 0 && legacyServices.length === 0) {
    return [`${chalk.bold('Automatic startup:')} ${muted('(not enabled yet)')}`];
  }
  ...
  for (const service of entries) {
    ...
    const headerSuffix = headerParts.length > 0
      ? `${muted('(')}${headerParts.join(muted(' • '))}${muted(')')}`
      : '';
    lines.push(`  • ${chalk.bold(service.name)}${headerSuffix ? ` ${headerSuffix}` : ''}`);

    const detailParts: string[] = [];
    if (service.targetMode) detailParts.push(`${muted('startup:')} ${formatTargetMode(service.targetMode)}`);
    if (typeof service.running === 'boolean') {
      detailParts.push(`${muted('running:')} ${service.running ? chalk.green('yes') : chalk.gray('no')}`);
    }
    if (service.configuredCliVersion) {
      detailParts.push(`${muted('cli:')} ${service.configuredCliVersion}`);
    }
    if (service.runningCliVersion && service.runningCliVersion !== service.configuredCliVersion) {
      detailParts.push(`${muted('running cli:')} ${service.runningCliVersion}`);
    }
    if (detailParts.length > 0) {
      lines.push(`${muted('    • ')}${detailParts.join(muted(' • '))}`);
    }
    if (service.path) {
      lines.push(`${muted('    • Installed at:')} ${muted(service.path)}`);
    }
  }
  return lines;
}
...
function formatCurrentDaemonStatusLines(params: Readonly<{
  ...
  const headerSuffix = relayProfile ? muted(`(relay profile: ${relayProfile})`) : '';
  const lines = [headerSuffix ? `${chalk.bold('Daemon:')} ${headerSuffix}` : chalk.bold('Daemon:')];

  const runningValue = daemon.running ? chalk.green('yes') : chalk.gray('no');
  lines.push(
    daemon.pid != null
      ? `  • ${muted('Running now:')} ${runningValue} ${muted(`(pid ${daemon.pid})`)}`
      : `  • ${muted('Running now:')} ${runningValue}`,
  );

  const noteMultipleDaemons = muted('  • Note: multiple daemons can run at once; use `happier daemon status --all` to list other relay profiles.');
  if (daemon.running !== true) {
    lines.push(noteMultipleDaemons);
    return lines;
  }

  if (daemon.serviceManaged === true) {
    lines.push(`  • ${muted('Started by:')} ${muted('automatic startup')}`);
  } else if (daemon.serviceManaged === false) {
    lines.push(`  • ${muted('Started by:')} ${muted('manual daemon start')}`);
  } else {
    lines.push(`  • ${muted('Started by:')} ${muted('unknown')}`);
  }

  if (daemon.startedWithPublicReleaseChannel || daemon.startedWithCliVersion) {
    lines.push(`  • ${muted('Running CLI:')} ${[
      daemon.startedWithPublicReleaseChannel ? formatReleaseChannel(daemon.startedWithPublicReleaseChannel) : muted('unknown'),
      daemon.startedWithCliVersion ?? muted('unknown'),
    ].join(muted(' • '))}`);
  }
  lines.push(noteMultipleDaemons);

  if (params.daemonCurrentInvocationMatches === false) {
    ...
    if (daemon.serviceManaged === true) {
      lines.push(muted('    Tip: run `happier service restart` to restart automatic startup with this installation.'));
    } else {
      lines.push(muted('    Tip: run `happier daemon restart --takeover` to restart this daemon with this installation.'));
    }
  }
...
export function renderServiceRepairRuntimeSummary(params: Readonly<{
  ...
  const relayLines = relays.length > 0
    ? formatDoctorLocalRelayLines(relays, {
