...
describe('codexAcp npm invocation (Windows)', () => {
  it('wraps npm view with cmd.exe on win32 (PATHEXT .CMD)', async () => {
    ...
    const binDir = await mkdtemp(join(tmpdir(), 'happier-codex-acp-bin-'));
    tempDirs.add(binDir);
    ...
    const { getCodexAcpDepStatus } = await import('./codexAcp');
    await expect(getCodexAcpDepStatus({ includeRegistry: true })).resolves.toEqual(
  ...
  it('wraps npm install with cmd.exe on win32 (PATHEXT .CMD)', async () => {
    ...
    const home = await mkdtemp(join(tmpdir(), 'happier-codex-acp-install-home-'));
    tempDirs.add(home);
    process.env.HAPPIER_HOME_DIR = home;

    const binDir = await mkdtemp(join(tmpdir(), 'happier-codex-acp-install-bin-'));
    tempDirs.add(binDir);
    await writeFile(join(binDir, 'npm.CMD'), '@echo off\r\necho ok\r\n', 'utf8');
    process.env.PATH = binDir;
    process.env.PATHEXT = '.CMD';
