 ...
 * close-after-select owner. The inline `WindowsRemoteSessionLaunchModeChip`
 * (rendered via `chip.render(ctx)`) must NOT close itself in its wrapper-level
 * `onSelect`. Closing happens only via `onRequestClose`.
 */
describe('WindowsRemoteSessionLaunchModeChip (inline render path)', () => {
    it('wrapper-level onSelect is a no-op and does NOT close the popover synchronously', async () => {
        ...
        await screen.pressByTestIdAsync('agent-input-windows-launch-mode-chip');
