...
vi.mock('react-native-unistyles', () => ({
  ...
  StyleSheet: { create: (value: any) => (typeof value === 'function' ? value({ colors: { divider: '#222', surface: '#000', surfaceHigh: '#111' } }) : value) },
}));
...
describe('SessionFileDetailsView (preview too large)', () => {
  it('renders a download action instead of a fatal error state', async () => {
    ...
    await act(async () => {
      tree = renderer.create(<SessionFileDetailsView sessionId="s1" scopeId="session:s1" filePath="big.txt" />);
    });

    await act(async () => {});

    expect(refreshSpy).toHaveBeenCalled();
    expect(tree.root.findAllByType('FileErrorState' as any).length).toBe(0);
    expect(tree.root.findAllByType('FileHeader' as any).length).toBe(1);
    expect(tree.root.findAllByProps({ testID: 'file-preview-unavailable-banner' }).length).toBe(1);
